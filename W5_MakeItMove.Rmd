...
1. *Why does it make sense to have a log10 scale on x axis?*
  ...
  Because the x-axis displays GDP per capita, and a log scale does a better job
  of showing percentage differences, e.g. increases in GDP of 10%, 50%, 100%, etc.
  If it was on a linear scale, the lower GDP countries would be more bunched
  together and harder to differentiate.
...
ggplot(subset(gapminder, year == 2007), aes(gdpPercap, lifeExp, size = pop, color = continent)) +
  scale_size_continuous(labels = scales::unit_format(unit = "", scale = .000001)) +
...
Solution adapted from https://stackoverflow.com/questions/65838876/how-to-edit-scale-in-ggplot2-legend
...
Starting from the richest, they are Norway, Kuwait, Singapore, United States and Ireland.
...
```{r}
nordics <- gapminder %>% filter(country=='Finland' | country =='Sweden' | country =='Denmark'| country =='Iceland' | country =='Norway')
```


```{r}
anim3 <- ggplot(nordics, aes(gdpPercap, lifeExp, color = country)) +
  geom_point(size = 10, alpha = 0.75) +
  labs(title = "Evolution of GDP and life expectancy in the Nordics\nYear: {frame_time}")+
  scale_x_log10() + # convert x to log scale
  transition_time(year)+
  xlab('GDP per capita')+
  ylab('Life expectancy')
...
How did GDP and life expectancy evolve in the Nordic countries between 1952-2007?

What stands out the most in the visualization is how Finland was significantly 
behind in the '50s and '60s, and started catching up between 1975-1985.